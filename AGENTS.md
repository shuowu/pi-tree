...
| `config` | `jinaApiKey?` |
| `cursors` | `get(userId, streamKeys[])`, `set(userId, entries[])`, `getByPrefix(userId, prefix)` |
...
- `message_usage` — per-message AI token consumption tracking (input/output/cache tokens, model, provider, cost). Cascades on session delete. Indexes on `session_id` and `created_at`
- `content_cursors` — generic per-user content stream position tracking. Keyed by `(user_id, stream_key)` where `stream_key` is plugin-namespaced (e.g. `news/feed/hackernews`). Used by plugins to track read watermarks and avoid showing duplicate content. Cascades on user delete.

Schema: `packages/server/src/db/schema.ts`. Migrations: `packages/server/drizzle/*.sql` (auto-generated by `drizzle-kit generate`, auto-applied on startup via `migrate()` from drizzle-orm). Applied migrations tracked in `__drizzle_migrations` table. See `db-migration` skill for full workflow.
...
**Usage API** (`/api/usage`):
- `GET /api/usage/session/:sessionId` — token usage for a specific session
- `GET /api/usage/:userId` — all-time usage for a user (supports `?from=` and `?to=` date filters)
- `GET /api/usage/:userId/:sourceId` — usage for a user+source

Returns `UsageStats`: `{ inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens, totalTokens, messageCount, costTotal?, byModel }`.
